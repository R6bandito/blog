...
// 代码高亮配色本地化（atom-one-dark，替换 jsdelivr CDN）
...
// 点击礼花效果（Canvas 轻量粒子）
hexo.extend.injector.register('body_end', "<script>(function(){if(window.__confettiInit){return;}window.__confettiInit=true;var cv=document.createElement('canvas');cv.style.cssText='position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:9999';document.body.appendChild(cv);var ctx=cv.getContext('2d');var dpr=Math.min(window.devicePixelRatio||1,2);function resize(){cv.width=innerWidth*dpr;cv.height=innerHeight*dpr;ctx.setTransform(dpr,0,0,dpr,0,0);}resize();addEventListener('resize',resize);var ps=[],raf=null;var COLORS=['#ff6fb3','#ffa726','#26c6da','#66bb6a','#ab47bc','#ffca28','#ef5350','#ff8a65'];function loop(){ctx.clearRect(0,0,cv.width,cv.height);for(var i=ps.length-1;i>=0;i--){var p=ps[i];p.vy+=0.28;p.x+=p.vx;p.y+=p.vy;p.vx*=0.99;p.rot+=p.vr;p.life-=0.016;if(p.life<=0||p.y>innerHeight+40){ps.splice(i,1);continue;}ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.rot);ctx.globalAlpha=Math.max(0,Math.min(1,p.life));ctx.fillStyle=p.color;ctx.fillRect(-p.w/2,-p.h/2,p.w,p.h);ctx.restore();}if(ps.length){raf=requestAnimationFrame(loop);}else{raf=null;ctx.clearRect(0,0,cv.width,cv.height);}}document.addEventListener('click',function(e){for(var i=0;i<12;i++){var a=Math.PI*2*Math.random();var sp=2+Math.random()*5;ps.push({x:e.clientX,y:e.clientY,vx:Math.cos(a)*sp,vy:Math.sin(a)*sp-2.5,rot:Math.random()*Math.PI,vr:(Math.random()-0.5)*0.35,w:3+Math.random()*4,h:5+Math.random()*5,color:COLORS[Math.floor(Math.random()*COLORS.length)],life:1});if(ps.length>240)ps.shift();}if(!raf){raf=requestAnimationFrame(loop);}},true);})();</script>");

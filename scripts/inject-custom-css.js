...
hexo.extend.injector.register('body_end', "<script>(function () { var mx = 0, my = 0, moved = false; document.addEventListener('touchstart', function (e) { moved = false; if (e.touches && e.touches[0]) { mx = e.touches[0].clientX; my = e.touches[0].clientY; } }, true); document.addEventListener('touchmove', function (e) { if (e.touches && e.touches[0] && (Math.abs(e.touches[0].clientX - mx) > 10 || Math.abs(e.touches[0].clientY - my) > 10)) { moved = true; } }, true); function handleItem(e) { if (e.type === 'touchend' && moved) { return; } var item = e.target && e.target.closest ? e.target.closest('.searchbox-result-item') : null; if (!item) { return; } var href = item.getAttribute('href'); if (!href) { return; } e.preventDefault(); e.stopPropagation(); var box = document.querySelector('.searchbox'); if (box) { box.classList.remove('show'); } var nav = document.querySelector('.navbar-main'); if (nav) { nav.style.pointerEvents = 'none'; setTimeout(function () { nav.style.pointerEvents = 'auto'; }, 400); } if (window.__pjaxNavigate) { window.__pjaxNavigate(href); } else { location.href = href; } } document.addEventListener('click', handleItem, true); document.addEventListener('touchend', handleItem, true); })();</script>");
